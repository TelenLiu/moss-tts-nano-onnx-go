...
func RewriteHyphensBeforeZhWeText(text string) string {
	...
	rewritten = reStartNeg.ReplaceAllString(rewritten, "${1}"+keepHyphenPlaceholder)
	reAfterDelim := regexp.MustCompile(`([=:+*/,(，:：；;（【\[{])\s*-\s*(?=\d)`)
	rewritten = reAfterDelim.ReplaceAllString(rewritten, "${1}"+keepHyphenPlaceholder)
	reZhNeg := regexp.MustCompile(`([\u3400-\u9fff])\s*-\s*(?=\d)`)
	rewritten = reZhNeg.ReplaceAllString(rewritten, "${1}"+keepHyphenPlaceholder)
	reNumRange := regexp.MustCompile(`(\d)\s*-\s*(?=\d)`)
	rewritten = reNumRange.ReplaceAllString(rewritten, "${1}"+keepHyphenPlaceholder)
	reZhPhrase := regexp.MustCompile(`([\u3400-\u9fff])\s*-\s*(?=[\u3400-\u9fff])`)
	rewritten = reZhPhrase.ReplaceAllString(rewritten, "$1，")
	reOther := regexp.MustCompile(`([^\s-])\s*-\s*(?=[^\s-])`)
	rewritten = reOther.ReplaceAllString(rewritten, "$1 ")
